  ...
  */
  ...
  function createPatchFunction(backend) {
    ...
    function updateChildren(parentElm, oldCh, newCh, insertedVnodeQueue, removeOnly) {
      ...
      let oldKeyToIdx, idxInOld, vnodeToMove, refElm; // removeOnly 是仅使用 <transition-group> 时的一个特殊标志，
      ...
      while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
        if (isUndef(oldStartVnode)) {
          ...
          patchVnode(oldStartVnode, newStartVnode, insertedVnodeQueue, newCh, newStartIdx); // oldStartVnode 和 newStartVnode 向右移动

          oldStartVnode = oldCh[++oldStartIdx];
          newStartVnode = newCh[++newStartIdx];
        } else if (sameVnode(oldEndVnode, newEndVnode)) {
          // oldEndVnode 和 newEndVnode 是同一个节点，则进行比较
          patchVnode(oldEndVnode, newEndVnode, insertedVnodeQueue, newCh, newEndIdx); // oldEndVnode 和 newEndVnode 向左移动
          ...
          canMove && // 插入元素
          nodeOps.insertBefore(parentElm, oldStartVnode.elm, nodeOps.nextSibling(oldEndVnode.elm)); // oldStartVnode 向右移动，newEndVnode 向左移动
          ...
          canMove && // 插入元素
          nodeOps.insertBefore(parentElm, oldEndVnode.elm, oldStartVnode.elm); // oldEndVnode 向左移动，newStartVnode 向右移动
          ...
          idxInOld = isDef(newStartVnode.key) ? oldKeyToIdx[newStartVnode.key] // 找出 oldCh（旧节点数组）中和 newStartVnode 是同一个节点的元素，然后返回其索引
          : findIdxInOld(newStartVnode, oldCh, oldStartIdx, oldEndIdx); // 判断 idxInOld 是否存在
          ...
          if (isUndef(idxInOld)) {
            ...
            // 取出当前需要比较的旧子节点
            vnodeToMove = oldCh[idxInOld]; // 是否为相同节点

            if (sameVnode(vnodeToMove, newStartVnode)) {
              patchVnode(vnodeToMove, newStartVnode, insertedVnodeQueue, newCh, newStartIdx); // 更新比较后，排除当前旧子节点即设置为 undefined，不在比较它。
      ...
      } // 退出判断循环后（即新或旧节点数组有一个已被查找完），若 oldStartIdx > oldEndIdx，
    ...
    function patchVnode(oldVnode, vnode, insertedVnodeQueue, ownerArray, index, removeOnly) {
      ...
      const ch = vnode.children; // vnode 中的 data 和 tag 属性同时为真，则对其属性（class、style等）进行更新
    ...
    return function patch(oldVnode, vnode) {
      ...
      if (isUndef(oldVnode)) ; else {
        // 是否为真实元素
        const isRealElement = isDef(oldVnode.nodeType); // 不是真实元素且新旧虚拟节点是同一个对象，则进行修补更新现有的根节点

        if (!isRealElement && sameVnode(oldVnode, vnode)) {
          console.log('开始-----------了'); // 修补现有的根节点
